...
func (s *Server) RunSimLoop(iterations int) {
	...
	// fmt.Println("Post founding Institutions")
	// for each megabike, print the governance method and the agents on it
	for _, bike := range s.GetMegaBikes() {
		fmt.Println("Founding Institutions: Bike ", bike.GetID(), " has governance ", bike.GetGovernance(), " and has agents ", bike.GetAgents())
	}
...
func (s *Server) FoundingInstitutions() {
	...
	fmt.Println("foundingTotals is ", foundingTotals)
	for governanceMethod, numBikers := range foundingTotals {
		...
		// fmt.Println("Bikes governed by ", governanceMethod, " are ", govBikes[governanceMethod], ", foundingTotals is ", foundingTotals, ", s.foundingChoices is ", s.foundingChoices)
		for agent, governance := range s.foundingChoices {
			...
			fmt.Println("agent ", agent, " has been added to bike ", chosenBike, " with governance ", governance)
			fmt.Println("bike ", chosenBike, " has ", len(s.GetMegaBikes()[chosenBike].GetAgents()), " agents")
		}
	}
