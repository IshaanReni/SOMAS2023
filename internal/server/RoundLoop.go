...
func (s *Server) RunRoundLoop() {
	...
	// fmt.Println("Starting round loop")

	// take care of agents that want to leave the bike and of the acceptance/ expulsion process
	// fmt.Println("RunBikeSwitch start")
	s.RunBikeSwitch(gameState)
	// fmt.Println("RunBikeSwitch end")

	// get the direction decisions and pedalling forces
	// fmt.Println("RunActionProcess start")
	s.RunActionProcess()
	// fmt.Println("RunActionProcess end")

	// The Audi makes a decision
	// fmt.Println("UpdateGameState start")
	s.audi.UpdateGameState(gameState)
	// fmt.Println("UpdateGameState end")
	...
	for _, bike := range s.megaBikes {
		...
		// fmt.Println("UpdateMass & MovePhysicsObject bike ", bike.GetID())
	}

	// Move the audi
	s.MovePhysicsObject(s.audi)
	// fmt.Println("MovePhysicsObject Audi")

	// Check Audi collision
	s.AudiCollisionCheck()
	// fmt.Println("AudiCollisionCheck")

	// Lootbox Distribution
	s.LootboxCheckAndDistributions()
	// fmt.Println("LootboxCheckAndDistributions")

	// Punish bikeless agents
	s.punishBikelessAgents()
	// fmt.Println("punishBikelessAgents")

	// Check if agents died
	s.unaliveAgents()
	// fmt.Println("unaliveAgents")

	// Replenish objects
	if utils.ReplenishLootBoxes {
		s.replenishLootBoxes()
		// fmt.Println("replenishLootBoxes")
	}
	if utils.ReplenishMegaBikes {
		s.replenishMegaBikes()
		// fmt.Println("replenishMegaBikes")
	}
...
func (s *Server) RunBikeSwitch(gameState GameStateDump) {
	...
	// fmt.Println("RunBikeSwitch changeBike	: ", changeBike)
	inLimbo = append(inLimbo, changeBike...)
	//process the kickout request
	// fmt.Println("RunBikeSwitch HandleKickoutProcess start")
	kickedOff := s.HandleKickoutProcess() // Da problem
	// fmt.Println("RunBikeSwitch kickedOff	: ", kickedOff)
	inLimbo = append(inLimbo, kickedOff...)
	// process the joining request
	// fmt.Println("RunBikeSwitch ProcessJoiningRequests start")
	s.ProcessJoiningRequests(inLimbo)
	// fmt.Println("RunBikeSwitch ProcessJoiningRequests	: ", changeBike)

}

func (s *Server) HandleKickoutProcess() []uuid.UUID {
	allKicked := make([]uuid.UUID, 0)
	// fmt.Println("HandleKickoutProcess start")
	// fmt.Println("HandleKickoutProcess s.GetMegaBikes()	: ", s.GetMegaBikes())
	for _, bike := range s.GetMegaBikes() {
		...
		case utils.Democracy:
			...
			// fmt.Println("HandleKickoutProcess Democracy ", bike.GetID())
			// fmt.Println("HandleKickoutProcess bike.GetAgents()	: ", bike.GetAgents())
			agents := bike.GetAgents()
			weights := make(map[uuid.UUID]float64)
			for _, agent := range agents {
				weights[agent.GetID()] = 1.0
			}
			// fmt.Println("HandleKickoutProcess weights	: ", weights)
			// get which agents are getting kicked out
			agentsVotes = bike.KickOutAgent(weights) // da problem
			// fmt.Println("HandleKickoutProcess agentsVotes	: ", agentsVotes)

		case utils.Leadership:
			// get the map of weights from the leader
			// fmt.Println("HandleKickoutProcess Leadership")
		...
		case utils.Dictatorship:
			...
			// fmt.Println("HandleKickoutProcess Dictatorship")
		...
		// fmt.Println("HandleKickoutProcess end for bike ", bike.GetID())
