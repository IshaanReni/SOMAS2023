...
func (mb *MegaBike) KickOutAgent(weights map[uuid.UUID]float64) []uuid.UUID {
	...
	// fmt.Println("weights inside kickOutAgent (megabike.go)")
	for _, agent := range mb.agents {
		// fmt.Println("agent inside kickOutAgent (megabike.go)")
		// fmt.Println("agentVotes inside kickOutAgent (megabike.go)", len(mb.agents))
		// fmt.Println("agentVotes agentID", agent.GetGameState().GetMegaBikes()[agent.GetBike()].GetAgents())

		agentVotes := agent.VoteForKickout() // Assuming this now returns map[uuid.UUID]int] // Also fucked up

		// fmt.Println("agentVotes inside kickOutAgent (megabike.go)", len(agentVotes))
		for agentID, votes := range agentVotes {
			// fmt.Println("agentID inside kickOutAgent (megabike.go)")
