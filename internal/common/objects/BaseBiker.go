...
// Returns the other agents on your bike :) // TODO: does not work!!!
func (bb *BaseBiker) GetFellowBikers() []IBaseBiker {
	bikes := bb.gameState.GetMegaBikes()
	// fmt.Println("bikeID in GetFellowBikers (BaseBiker.go) ", bb.GetBike())
	bike := bikes[bb.GetBike()]
	// fmt.Println("bike in GetFellowBikers (BaseBiker.go) ", bike)
	fellowBikers := bike.GetAgents()
	// fmt.Println("fellowBikers in GetFellowBikers (BaseBiker.go) ", fellowBikers)
	return fellowBikers
}
...
func (bb *BaseBiker) VoteForKickout() map[uuid.UUID]int {
	...
	// fmt.Println("bikeID in VoteForKickout (BaseBiker.go) ", bikeID)

	// bikeState := bb.GetGameState().GetMegaBikes()[bikeID]
	// fmt.Printf("bikeState in VoteForKickout (BaseBiker.go) %+v\n", bikeState)
	// ellowBikers := bb.GetFellowBikers() // TODO: Where it fucks up fr
	// fmt.Printf("ellowBikers in VoteForKickout (BaseBiker.go) %+v\n", ellowBikers)
	...
	for _, agent := range fellowBikers {
		...
		if agentID != bb.GetID() {
			...
			// fmt.Println("agentID in VoteForKickout (BaseBiker.go) ", agentID, " with vote ", voteResults[agentID])
		}
	}
